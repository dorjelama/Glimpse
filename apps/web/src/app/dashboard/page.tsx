...
function ProjectCard({
  ...
  return (
    <div
      ...
      className="group relative bg-cream border border-gold/30 rounded-2xl overflow-hidden cursor-pointer hover:border-terra/40 hover:shadow-md hover:shadow-terra/10 transition-all duration-200"
    >
      {/* Preview thumbnail */}
      <div
      ...
      {/* Info */}
      <div className="p-4">
        <p className="text-ink font-medium text-sm truncate">{project.title}</p>
        {project.date && (
          <p className="text-[11px] text-ink/40 mt-0.5">{formatDate(project.date)}</p>
        )}
        ...
        <p className="text-[11px] text-ink/30 mt-2">Edited {timeAgo(project.updatedAt)}</p>
      </div>
        ...
        <button
          ...
          className={`text-[11px] px-2.5 py-1 rounded-lg transition-colors ${
            confirmDelete
              ...
              : 'bg-cream/80 border border-gold/30 hover:bg-red-50 hover:border-red-200 text-ink/40 hover:text-red-500'
          }`}
        >
  ...
  return (
    <div className="bg-blush/50 border border-gold/20 rounded-2xl overflow-hidden animate-pulse">
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="h-3 bg-ink/5 rounded w-1/3 mt-3" />
      </div>
    </div>
  );
}
...
function WelcomeModal({ onGetStarted, onSkip }: { onGetStarted: () => void; onSkip: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm px-4">
      <div className="bg-cream border border-gold/30 rounded-2xl shadow-xl w-full max-w-md p-7 flex flex-col gap-6">
        <div>
          <p className="text-xs font-bold tracking-widest uppercase text-terra mb-3">Glimpse</p>
          <h2 className="text-ink font-bold text-xl leading-snug" style={{ fontFamily: 'Georgia, serif' }}>
            Create events your guests will remember.
          </h2>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-blush border border-gold/20 rounded-xl p-4 flex flex-col gap-2">
            <span className="text-2xl">✦</span>
            <p className="text-ink font-semibold text-sm">Cards</p>
          ...
          <div className="bg-blush border border-gold/30 rounded-xl p-4 flex flex-col gap-2">
            <span className="text-2xl">📸</span>
        ...
        <div className="flex flex-col gap-2">
          <button
            ...
            className="w-full py-3 bg-terra hover:bg-terra/90 text-white font-semibold text-sm rounded-xl transition-colors"
          >
            Create your first event →
          </button>
          <button
            onClick={onSkip}
            className="w-full py-2 text-sm text-ink/40 hover:text-ink/70 transition-colors"
          >
...
function NewEventModal({ onCreate, onClose }: { onCreate: (title: string, date?: string) => Promise<void>; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-cream border border-gold/30 rounded-2xl shadow-xl w-[400px] max-w-full mx-4 p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-ink font-semibold text-base mb-1" style={{ fontFamily: 'Georgia, serif' }}>New Event</h2>
        <p className="text-ink/50 text-sm mb-4">Give your event a name and date. You can change these later.</p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          <input
            ...
            className="w-full bg-blush text-ink text-sm rounded-xl px-4 py-3 border border-gold/40 focus:outline-none focus:border-terra placeholder:text-ink/30 transition-colors"
          />
          <input
            type="date"
            value={date}
            onChange={e => setDate(e.target.value)}
            className="w-full bg-blush text-ink text-sm rounded-xl px-4 py-3 border border-gold/40 focus:outline-none focus:border-terra transition-colors [color-scheme:light]"
          />
          <div className="flex gap-2 justify-end mt-1">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm text-ink/40 hover:text-ink transition-colors">
              Cancel
            </button>
            <button
              type="submit"
              disabled={creating || !title.trim()}
              className="px-4 py-2 bg-terra hover:bg-terra/90 disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-colors"
            >
      ...
      <div className="min-h-full bg-cream px-4 py-6 md:px-8 md:py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-ink/40 mt-0.5">All your events in one place</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowWelcome(true)}
              title="What is Glimpse?"
              className="w-8 h-8 flex items-center justify-center rounded-xl bg-blush hover:bg-blush/80 text-ink/40 hover:text-ink text-sm font-bold transition-colors border border-gold/30"
            >
              ?
            </button>
            <button
              onClick={() => setShowModal(true)}
              className="flex items-center gap-2 px-4 py-2 bg-terra hover:bg-terra/90 text-white text-sm font-semibold rounded-xl transition-colors shadow-sm"
            >
